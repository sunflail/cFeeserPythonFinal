        ...
        for rover in rovers:
            ...
            with open("./roverpics.txt", "a") as output:
                ...
                for x in roverreq['photos']:
                    output.write(f"{x['rover']['name']} - {x['camera']['full_name']} - -{x['earth_date']} - {x['img_src']}\n")
...
def showApod(date):
    ...
    last_year_bday = findRecentBday(date)
...
def configEndDayForNEOW(startdate):
    """
...
def nasaNEOWRequest(enddate, startdate):
    """
...
def main():
    ...
    ## configure end date
    enddate = configEndDayForNEOW(startdate)
    ## perform the actual api call
    neowdata = nasaNEOWRequest(enddate, startdate)
    ...
    if input("Would you like to see the Nasa Pic of the Day from last years same day and month? (y/n)").lower() == 'y':
        showApod(startdate)
    if input("Would you like to have a text file with links to pictures from all three Mars Rovers for a random sol of their mission? (y/n)").lower() == 'y':
        marsRover()
